...
本软件既**对小白极其友好**（支持极简的一键拖拽翻译），又非常适合有一定基础的用户上手极客部署或二次开发。

通过原生的注入方式，在游戏运行期间实时抓取文本，并使用强大的 LLM (AI大语言模型) 引擎将其翻译为您需要的语言，浮窗覆盖显示在游戏之上。

## ✨ 核心特性

- **拖拽式一键操作**: 直接将游戏的 `.exe` 文件拖入主界面即可。
- **动态 Hook 注入**: 原生文本提取。通过 Hook 脚本 (`_translator_hook.rpy`) 直接桥接 Ren'Py 游戏引擎，实现无损文本提取。
- **多引擎支持 (LLMs)**: 本地和云端大语言模型无缝切换，保证上下文连贯性和角色语气：
  ...
  - **Ollama**: 本地离线模型支持，保护隐私且零延迟。
  - **自定义节点**: 支持兼容 OpenAI 格式的第三方/自建 API 端点接入。
- **透明覆盖层 UI**: 翻译结果显示在一个无边框、可拖动、可调节的浮窗上，不干扰游戏的原始画面。
- **缓存与防抖机制**: 内置 SQLite + 内存双层独立缓存（按游戏隔离）。同时支持防抖，避免玩家快速“快进 (Ctrl/Skip)”时的无效 API 滥用和排队。

## 🎮 软件使用

1. **获取软件**:
   - 您可以直接点击右侧的 **Releases** 页面下载最新版的 `RenpyLens.exe`。
   - 或者，您也可以按照下方的 **[🛠️ 代码开发](#-代码开发)** 步骤，自行克隆代码并使用 Python 运行或打包。
2. **配置 API / 模型**: 
   - **无自有 API / 极简操作用户**: 直接双击 `.exe` 打开软件，在翻译引擎中选择 **"内置通道"**，点击主界面的 **"🔑 获取试用 API"**。获取成功后即可直接导入游戏体验。
   - **自带 API / 高阶用户**: 点击软件界面的 **“⚙️ 设置”** 按钮。在 **“API 设置”** 选项卡中，填入你想使用的 LLM 服务商密钥及相关配置（例如 Gemini 或 Ollama 的本地地址端口）。
3. **选择游戏**:
   - 将你需要翻译的 Ren'Py 游戏的主程序 `.exe` 直接拖拽到软件界面的指定区域。
4. **注入并启动**:
   - 拖入后，点击 **"▶ 开始游戏"**，工具会自动将所需的脚本放入游戏的 `game/` 文件夹下。
   - 游戏启动后，翻译浮窗会自动弹出。当游戏中出现新对话时，浮窗会实时显示翻译结果。
5. **浮窗控制**:
   - **拖动位置**: 鼠标左键按住浮窗上的 **文字部分** 拖动即可自由调整翻译界面在游戏上的位置。
   - **更多设置**: 鼠标 **右击文字** 即可唤出快捷菜单，进行字体大小调节、背景透明度设置等更多的自定义控制。
6. **清理与卸载**:
...
本项目采用 [GPLv3 License](LICENSE) 许可协议开源。您可以自由使用、修改和分发，但请务必开源衍生代码并保留原作者信息。
