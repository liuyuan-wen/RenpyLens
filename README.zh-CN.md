...
- **RPGM工具：**可按游戏选择文字速度、对话框透明度、选项回退、强制保存、高速移动、穿墙、一键战胜/战败等。
...
- **智能翻译：**新增 Ren'Py 分支感知预测和自适应批量预取，优先处理玩家选择的路线；自动重试并拒绝重复字符或异常过长的模型输出。
- **浮窗与 RPGM 工具：**新增原文和译文的一键复制与自动复制，优化多显示器长文本布局；新增对话回退，并改进强制保存和工具状态保存。
- **兼容性与打包：**改进 Ren'Py 动态角色名提取、Hook 预测覆盖、残留端口清理和 UPX 构建校验。

### v1.5.3

- **RPGM 随时保存：**新增可选的“随时保存”工具，可通过游戏内工具栏按钮或 `Ctrl+S` 打开原生保存界面。
- **长文本浮窗：**长篇屏幕文字支持滚动和调整大小，并新增浮窗位置重置入口，改善拖动与调整大小的交互体验。
- **引擎兼容性：**改进 Ren'Py 屏幕文字、角色和选项提取、旧版文本序列处理，以及 RPGM 换行处理。
...
1. **下载 RenpyLens**
   - 从仓库右侧的 **Releases** 页面下载最新版 `RenpyLens_v1.5.4.exe`。
   - 也可以按照下方开发说明从源码运行。
2. **选择翻译服务**
   - 若希望快速体验，选择 **内置通道**，然后点击 **获取试用 API**。
   - 若使用自己的服务，打开 **设置 → API 设置**，配置服务商、Ollama 或自定义 OpenAI 兼容通道。
3. **选择游戏**
   - 将 Ren'Py、RPG Maker MV/MZ 游戏的主程序 `.exe` 拖入软件窗口。
   - 导入 RPG Maker MV/MZ 时，可在翻译引擎与模型右侧开启“RPGM工具”，并从旁边的菜单勾选所需功能。
4. **安装 Hook 并启动**
...
打包产物为 `RenpyLens_v1.5.4.exe`。如果项目根目录存在 `upx.exe`，打包脚本默认会使用它进行压缩。如果生成的程序被杀毒软件拦截或无法启动，可以使用以下命令禁用 UPX 后重新打包：
